...
class FrontierAgent(MemoryAgent):
    ...
    def __init__(self, image_shape=None, decaying_factor=0.9, one_step=False, stochastic=False,
            ...
            th_forget=0.00, use_rrt=False, use_memory_list=True, surp4frag='stm', update_first=True, epsilon=5, use_heuristics=False, rho=2.0, frag_mode='z'):

        super(FrontierAgent, self).__init__(image_shape, decaying_factor, stochastic=stochastic, is_planning=is_planning, fragmentation=fragmentation, use_rrt=use_rrt, epsilon=epsilon, use_memory_list=False, rho=rho, frag_mode=frag_mode)
        self.head_bias = use_heuristics
        self.use_size = use_heuristics
