...
class MemoryAgent(RandomAgent):
    def __init__(self, image_shape=None, decaying_factor=0.9, one_step=False, stochastic=False,
            ...
            th_forget=0.00, use_rrt=False, use_memory_list=True, surp4frag='stm', update_first=True, epsilon=5, rho=2, frag_mode='z'):
        super(MemoryAgent, self).__init__(image_shape, is_planning, one_step, use_rrt=use_rrt)
        self.stochastic = stochastic
        self.fragmentation = fragmentation

        
        stm = MappingMemory(shape=image_shape, decaying_factor=decaying_factor, egocentric=True, discrete=True, fixed_cur_pos=(image_shape[-2]-1,image_shape[-1]//2))
        if use_memory_list:
        ...
        # create a new map
        self.new_stm = lambda x : MappingMemory(shape=image_shape, decaying_factor=decaying_factor, egocentric=True, discrete=True, fixed_cur_pos=(image_shape[-2]-1,image_shape[-1]//2))
    ...
    def update_score(self, location, obs, s_map=None):
        ...
        if 'stm' in self.surp4frag:
            mem_map = self.stm.get_map(head=d, dh=h, dw=w, state_shape=(512, self.image_shape[-2], self.image_shape[-1]))
            score = get_score(mem_map, obs, self.stm.pixel_value)
