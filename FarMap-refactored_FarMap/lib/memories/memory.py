...
class MappingMemory:
    def __init__(self, shape=(3,100,100), fixed_cur_pos=None, egocentric=False, decaying_factor=0, discrete=False, size_limit=(250,250), id=0, init_val_one=False):
    ...
    def update(self, state, action):
        ...
        if self.fixed_cur_pos is not None:
    ...
    def update_by_rel(self, state, rel_position, ignore_current=True):
        """
    ...
    def get_size(self):
        ...
        for pixel_key, idx in self.pixel_value.items():
            ...
            if np.all(pixel == 0):
                known.append(idx)
            elif not np.all((pixel - ord('X')/255) < 1e-6):
                known.append(idx)
...
if __name__ == '__main__':
    memory = MappingMemory(shape=(3,2,2), fixed_cur_pos=(4,2), decaying_factor=0.7, egocentric=True, discrete=True)
