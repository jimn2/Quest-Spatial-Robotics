...
from lib.maze_art import MAZES_ART
...
def check_discard(pixel):
    for p in pixel:
        if abs(p - -1/255) > 1e-6 and abs(p - ord('X') /255) > 1e-6:
            return False
    return True


def get_score(pred, gt, pixel_value):
    """
...
def get_ground_truth(h, w, d, obs, score_map, used_map, ratio=1):
    C, H, W = obs.shape

    score = get_obs(h, w, d, score_map, size=(H,W), padding_value=0)
    mask = get_obs(h, w, d, used_map, size=(H, W), padding_value=0)
...
def get_map(maze_id, num_envs=1, scale=3, generation=False, maze_path='gen_env.pkl', obs_size=(5,5)):
    mazes = []
    for _ in range(num_envs):

        if maze_id == -1: # specialized large empty room
            maze = np.zeros((3, 200, 200)) + 37
            maze[:, 1:-1, 1:-1] = 0
        elif generation:
        ...
        row, col = ((maze==255).sum(0)==3).nonzero()
        maze[:,row,col] = 0
        mazes.append(maze)
        ...
        for row in maze:
            ...
            for e in row:
                if e == 'P':
                    ...
                    new_row.append(ord(e)) # temp
            maze_map.append(new_row)
        maze_map = np.asarray(maze_map)
        coin = maze_map == 245
        maze_map = np.stack([maze_map for _ in range(3)])
        coin_coor = np.nonzero(coin)
        if len(coin_coor[0]) > 0:
            rows, cols = coin_coor
            for row, col in zip(rows, cols):
                maze_map[:, row, col] = np.asarray([245, 90, 90])
        maps.append(maze_map)
    return maps


def enlarge_map(maze, window, scale=1):
